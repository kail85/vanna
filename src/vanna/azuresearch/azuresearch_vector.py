...
class AzureAISearch_VectorStore(VannaBase):
    ...
    Args:
        config (dict): Configuration dictionary. Defaults to {}. You must provide an API key in the config.
            ...
            - dimensions (int, optional): Dimensions of the embeddings. Defaults to 1536 which corresponds to the dimensions of text-embedding-3-small.
            - text_embedding_model (str, optional): text embedding model to use. Defaults to "text-embedding-3-small".
    ...
    def __init__(self, config=None, client=None):
        """
        - client (AzureOpenAI, optional): AzureOpenAI client. Defaults to None.
        """
        VannaBase.__init__(self, config=config)

        if client is None:
            raise ValueError(
                "client is required, pass an AzureOpenAI client in the client parameter."
            )
        self.client = client
        ...
        self.text_embedding_model = config.get(
    ...
    def generate_embedding(self, data: str, **kwargs) -> List[float]:
        return (
            self.client.embeddings.create(input=[data], model=self.text_embedding_model)
            .data[0]
            .embedding
        )
